...
class attendance extends Component {
  ...
  componentDidMount() {
    ...
    this.getSchedulesByDepartmentId()
  }
  getAllSubject = () => {
  ...
  getSchedulesByDepartmentId = () => {
    ...
    Axios.get(`/api/schedule/department/${dataUser[0].id_department}`)
      .then((res) => {
        if (res.status === 200) {
        ...
        if (res.status === 200) {
          ...
          for (let i = 0; i < hihi.length; i++) {
            ...
            if (getdate1 < 6) {
              ...
              var dayWork =
                (Date.parse(endDay) - Date.parse(firstDay)) /
                  (1000 * 60 * 60 * 24) +
                1
          ...
          for (let i = 0; i < hihi.length; i++) {
            ...
            if (getdate1 == 6) {
              ...
              salaryOTEachDay =
                salaryOT + (salaryofEmployee / 26 / 8) * totalOT * (150 / 100)
          ...
          for (let z = 0; z < hihi.length; z++) {
            ...
            for (let i = 0; i < shiftLate.length; i++) {
              ...
              if (mathTimeLate > 60) {
                salaryLateEach =
                  salaryLate + Math.floor(salaryofEmployee / 26 / 2)
                // salaryLate = salaryLateEach
                totalLateIn.push(salaryLateEach)
              }
              salaryLate = salaryLateEach
          ...
          const slice = this.state.attendanceAll.slice(
            this.state.offset,
          ...
          this.setState({
            pageCount: Math.ceil(
              this.state.attendanceAll.length / this.state.perPage
            ),
            postData,
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          this.setState({
            ...
            selectedShift: {
              value: listById.id_shift,
              label: listById.shift_name,
            },
          })
        }
      })
  ...
  handleInsertAttendance = (event) => {
    ...
    for (let z = 0; z < this.state.resignByLogin.length; z++) {
      var checkDate = moment(this.state.resignByLogin[z].date).format(
        'YYYY-MM-DD'
      )
    ...
    Axios.post('/api/attendance/edit', newEditPosition)
      .then((res) => {
        ...
        this.setState((prevState) => ({
          attendance: prevState.attendance.map((elm) =>
            elm.id_attendance === key
              ? {
                  ...elm,
  ...
  getListAccount = () => {
    ...
    Axios.get(`/api/subject/allStudents/${dataUser[0].id_department}`)
      .then((res) => {
        if (res.status === 200) {
                                  ...
                                  <tr key={key}>
                                    ...
                                    <th>
                                      <button
                                        ...
                                        data-toggle="modal"
                                        data-target="#formemodalattendance"
                                        onClick={() =>
                                          this.getShiftById(item.id_account) ||
                                          this.getDataResignById(
                                            item.id_account
                                          ) ||
                                          this.getAllAttendance(item.id_account)
                                        }
                                      >
                                        {' '}
                            ...
                            <div className="row">
                              ...
                              <div className="col-lg-12">
                                <h5 className="card-title">
                                  List All attendance of Employee
                                </h5>
                                <br />
                                <ExcelFile
                                  element={
                  ...
                  <>
                    ...
                    <div className="card">
                      <div className="container text-center">
                        <div className="row">
                          <div className="col">Thứ/Buổi</div>
                        ...
                        <div>
                          <div className="row">
                            ...
                            {Array.from([0,1,2,3,4]).map((item, idx) => {
                              const schedulesIdx = this.state.schedules.findIndex(ele => ele.day === idx)
                              if (schedulesIdx === -1) return <div className='col'>...</div>

                              if (this.state.schedules[schedulesIdx].id_shift === 1) return<div className='col'>
                                <button className="btn btn-light btn-block m-1"
                                    data-toggle="modal"
                                ...
                                {
                                this.state.schedules[schedulesIdx].id_subject === 1 ? 'Toán' : 'Văn'
                              }
                                  </button></div>
                          ...
                          <div className="row">
                            ...
                            {Array.from([0,1,2,3,4]).map((item, idx) => {
                              ...
                              if (this.state.schedules[schedulesIdx].id_shift === 2) return<div className='col'>
                                <button className="btn btn-light btn-block m-1"
                                    data-toggle="modal"
                                    data-target="#formemodal"
                                    onClick={() => {
                  ...
                  <>
                        ...
                        <div>
                          <div className="row">
                            ...
                            {Array.from([0,1,2,3,4]).map((item, idx) => {
                              ...
                              if (this.state.schedules[schedulesIdx].id_shift === 1) return<div className='col'>
                                {this.state.schedules[schedulesIdx].id_subject === 1 && 'Toán'}
                                {this.state.schedules[schedulesIdx].id_subject === 2 && 'Văn'}
                                {this.state.schedules[schedulesIdx].id_subject === 3 && 'Anh'}
                                {this.state.schedules[schedulesIdx].id_subject === 4 && 'Sinh'}
                              </div>
                    ...
                    <div className="card">
                      {this.state.student_late.map((item) => {
                        return (
                          <div>
                            Học sinh {dataUser[0].name} đã đi muộn ngày{' '}
                        ...
                        <div className="card-body">
                          <form>
                            ...
                            <button
                              type="submit"
                              className="btn btn-light px-5"
                              onClick={this.handleEditAttendance}
                            >
