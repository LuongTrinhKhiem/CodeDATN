...
class Bend extends Component {
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          this.setState({
            ...
            selectedDepartment: {
              value: listById.id_department,
              label: listById.department_name,
            },
            selectedSalary: {
              value: listById.id_salary,
              label: listById.money,
            },
            selectedShift: {
            ...
            selectedPosition: {
              value: listById.id_position,
              label: listById.position_name,
            },
            selectedRole: {
              value: listById.id_role,
              label: listById.role_name,
            },
          })
        }
      })
  ...
  handleInsertUser = (event) => {
    ...
    axios
      .post('/api/user/insert', newUser)
      .then((res) => {
        let news = this.state.news
        news = [newUser, ...news]
        this.setState({ news: news })
        swal('Yeahh! You have successfully added new user!', {
          icon: 'success',
        })
        this.modalClose()
      })
  ...
  handleEditUser = (event) => {
    ...
    axios
      .post('/api/user/edit', newEditUser)
      .then((res) => {
        console.log(res)
  ...
  deleteUser = (item) => {
    ...
    axios
      .post('/api/account/delete', accountId)

      .then((res) => {
        this.setState((prevState) => ({
          news: prevState.news.filter(
            (elm) => elm.id_account !== item.id_account
          ),
        }))
                            ...
                            <tr key={key}>
                              ...
                              <th>
                                <button
                                  ...
                                  onClick={() =>
                                    this.getAllUserById(item.id_account)
                                  }
                                >
                                  {' '}
                                ...
                                <button
                                  ...
                                  data-toggle="modal"
                                  data-target={
                                    '#modal-animation-' + item.id_account
                                  }
                                >
                                  {' '}
                                ...
                                <div
                                  className="modal fade"
                                  id={'modal-animation-' + item.id_account}
                                  style={{ display: 'none' }}
                                  aria-hidden="true"
                                >
