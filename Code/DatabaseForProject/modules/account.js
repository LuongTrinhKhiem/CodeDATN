...
module.exports = (app) => {
  ...
  app.get("/api/account/getById/(:id_account)", (req, res) => {
    ...
    var sql =
      "SELECT account.id_account,account.username, account.password, account.name, account.age, account.address, account.phone, department.department_name, department.id_department, salary.money, salary.id_salary, shift.shift_name,shift.id_shift, position.position_name, position.id_position, role.role_name, role.id_role FROM `account` INNER JOIN department ON account.id_department = department.id_department INNER JOIN salary ON account.id_salary = salary.id_salary INNER JOIN shift ON account.id_shift = shift.id_shift INNER JOIN position ON account.id_position = position.id_position INNER JOIN role ON account.id_role = role.id_role WHERE id_account ='" +
      id_account +
      "'";
    connection.query(sql, function (err, results) {
      if (err) throw err;
  ...
  app.post("/api/account/delete", (req, res) => {
    var sql =
      "DELETE FROM account " + "WHERE id_account='" + req.body.id_account + "'";
    connection.query(sql, function (err, results) {
      if (err) throw err;
  ...
  app.post("/api/user/edit", (req, res) => {
    const { id_account, department_name, money, shift_name, position_name, role_name, ...rest } = req.body;
    const sqel = squel.update().table("account");

    sqel.setFields(rest);

    sqel.where(`id_account = ${id_account}`);

    connection.query(sqel.toString(), function (err, results) {
      if (err) throw err;
      res.json({ department: results });
    });
  });
